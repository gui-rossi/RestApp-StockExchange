...
class cliente {
	...
	cliente() {
		...
		while (j < 1) {
			...
			cotacoes_interesse cotacoes_interesse = new cotacoes_interesse(cotacoes.acoes[i]);
			this.cotacoes_interesse.add(cotacoes_interesse);
...
class compra_venda {
	...
	compra_venda(int id_cliente, String tipo_negocio, String empresa, int qtde_acoes, int valor_MinimoMaximo){
...
public class ServImpl extends UnicastRemoteObject {
				...
				for (cotacoes_detem detem : clientes.cotacoes_detem) {
					...
					if(detem.acao.contains(empresa)) {
						...
						if (detem.qtde < qtde) {
							...
							compra_venda venda = new compra_venda(id, "venda", empresa, qtde, preco_minimo);
				...
				//for (cotacoes_detem detem : clientes.cotacoes_detem) {
					...
					//if(detem.acao.contains(empresa)) {
						...
						//se ele tem dinheiro pra comprar as acoes, faco o pedidod de compra
							...
							compra_venda compra = new compra_venda(id, "compra", empresa, qtde, preco_maximo);
							System.out.println(clientes.nome + " pedido de compra colocado");
						...
						if (compras_vendas.qtde_acoes < compras_vendas_aux.qtde_acoes) {
							for (cliente clientes : acionistas) {
								...
								if (clientes.id == compras_vendas.id_cliente) {
									...
									Server.id_vendedor = Integer.toString(clientes.id);
									Server.message_vendedor = LocalTime.now().toString() + " " + clientes.nome + " vendeu " + compras_vendas.qtde_acoes + " da " + compras_vendas.empresa;
								...
								//achou comprador: tira dinheiro da conta 
									...
									if (!ja_detia_acao) {
										...
										inserirCotacaoInteressante(compras_vendas_aux.empresa, clientes.id);
									}
									//notifico comprador de que ele comprou acoes
									Server.id_comprador = Integer.toString(clientes.id);
									Server.message_comprador = LocalTime.now().toString() + " " + clientes.nome + " comprou " + compras_vendas.qtde_acoes + " da " + compras_vendas_aux.empresa + ", ainda quero comprar " + (compras_vendas_aux.qtde_acoes - compras_vendas.qtde_acoes);
						...
						//2 se o vendedor vende mais acoes do que o comprador compra
							...
							for (cliente clientes : acionistas) {
								...
								if (clientes.id == compras_vendas.id_cliente) {
									...
									//notifico o vendedor que ele vendeu acoes
									Server.id_vendedor = Integer.toString(clientes.id);
									Server.message_vendedor = LocalTime.now().toString() + " " + clientes.nome + " vendeu " + compras_vendas_aux.qtde_acoes + " da " + compras_vendas.empresa + ", ainda quero vender " + (compras_vendas.qtde_acoes - compras_vendas_aux.qtde_acoes);
								...
								//achou comprador: tira dinheiro da conta, coloca mais acoes no nome dele
									...
									//notifico comprador de que ele comprou acoes
									Server.id_comprador = Integer.toString(clientes.id);
									Server.message_comprador = LocalTime.now().toString() + " " + clientes.nome + " comprou " + compras_vendas_aux.qtde_acoes + " da " + compras_vendas_aux.empresa;
						...
						//3 se o vendedor e o comprador compram acoes de mesma quantidade
							...
							for (cliente clientes : acionistas) {
								...
								//achou comprador: tira dinheiro da conta, coloca mais acoes no nome dele
									...
									//notifico comprador de que el comprou acoes
									Server.id_comprador = Integer.toString(clientes.id);
									Server.message_comprador = LocalTime.now().toString() + " " + clientes.nome + " comprou " + compras_vendas.qtde_acoes + " da " + compras_vendas_aux.empresa;
	...
	public String obterCotacoes (int id) throws RemoteException{
		String msg = "{ \"linha\": [";
				...
				if (this.acionistas.get(i).id == id)
				{
					...
					for (int j = 0; j < this.acionistas.get(i).cotacoes_interesse.size(); j++)
					{
						for (int k = 0; k < cotacao_global.num_acoes; k++)
						{
							...
							{
								//monto o JSON para envio do get
								if (firstTime)
								{
									msg = msg + "{\"acao\": " + "\"" + cotacao_global.acoes[k]+ "\"" + ", \"preco\": " + cotacao_global.precos[k];
									firstTime = false;
								}
								else
									msg = msg + "},{\"acao\": " + "\"" + cotacao_global.acoes[k] + "\"" + ", \"preco\": " + cotacao_global.precos[k];
				...
				for (int i = 0; i < cotacao_global.num_acoes; i++) {
					...
					if (cotacao_global.acoes[i].contains(notificacao.cotacoes)) {
						...
						if (notificacao.tipo.contains("perda") && cotacao_global.precos[i] - cotacao_global.multiplicador_importancia[i] < notificacao.limite) {
							...
							Server.id_notificar = Integer.toString(clientes.id);
							Server.message_notificar = LocalTime.now().toString() + " " + clientes.nome + " as acoes da " + notificacao.cotacoes + " atingiram o limite de " + notificacao.tipo + " (Cotacao atual: " + cotacao_global.precos[i] + ")";
							Server.hadNot = true;
    ...
    public void registrarInteresse(String nome, int id) throws RemoteException{
